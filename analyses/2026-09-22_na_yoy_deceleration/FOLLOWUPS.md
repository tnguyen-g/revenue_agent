...
| F2 | Checkout: the Tue/Mon drop is in purchases per buy-click on every platform (−4% to −7% vs a normal of about 0%). Tuesday matched the previous Tuesday, so this is mostly Monday strength (F14). Bring the checkout-completion chart and check the 403 RECAPTCHA rate after the 9/22 JPROD-955 change, plus the PayPal auth rate | Incident / tech | Ernesto Martin + Giovanni Lagasio; Checkout & Payments | 9/23 11:30 CEST meeting | Root cause named or ruled out; any lost orders/M1VFM quantified in JPROD-955 | sql/11; report section 4; JPROD-955; OPS NA Elastic check | In progress (funnel read done) |
| F3 | Non-Brand SEM orders −9.3% Tue/Mon vs −3.6% normal, with impressions about −8% Tue/Mon on app and touch. The same drop happened on 8/24 and 9/14, so it looks like a weekday delivery pattern. Confirm with the SEM team (weekday budget or bidding) and check whether the "remaining 25% to the new system" migration went live on 9/22 | Channel | Jakub Nguyen + SEM team *(proposed)* | 9/24 | Weekday pattern confirmed or fixed; go/no-go on the 50% step recorded | sql/08; adspend / roi_datamart_sem (needs access, F10) | Open |
...
| F10 | Data access: the FoundryAI BigQuery connector cannot query the dataview views (supply_unagi, finance_unit_economics, marketing incl. superfunnel, checkoutfunnel, product_analytics, managed_channels, pricing_and_promotions). The funnel was covered through queryable source and linked copies in `kbc-grpn-35` and `kbc-grpn-28`. Still missing: LY buy-clicks and checkout, bounce and sessions, checkout_conversion, adspend, email/push sends. Confirm the source-table path is acceptable, or request BigQuery Data Viewer / authorized-view access for the connector principal, or use the Ask Data MCP | Data | Jakub Nguyen → data platform / dataset owners | 9/25 | Checkout conversion, adspend and CRM sends queryable | PLAN.md §2 | Open |
...
| F13 | Browser checkout completion (purchases per buy-click) is below August: touch 24.9% vs 28–29%, web 30.2% vs 35–36%, apps flat. Size the gap in purchases per day (roughly 1–2K at August rates) after checking deal mix and seasonality. Break it down by browser, OS and payment method. Overlay reCAPTCHA threshold changes (JPROD-955/957), the PayPal REST cutover (9/15) and 3DS (JPROD-737) | Structural | Checkout & Payments; Jakub Nguyen for the RevMan sizing *(proposed)* | Sizing by 9/25; owner plan by 9/30 | Gap explained (mix) or a recovery plan with a target completion rate per platform | sql/11; data/checkout_funnel_ty_baseline.csv; report section 4 | Open |
| F14 | Explain the Monday 9/21 checkout spike: completion +5% to +17% vs Mon 9/14 on every platform. Was it the iOS reCAPTCHA threshold (live 9/20 17:28) or Monday checkout promos? This decides the right baseline for the Tuesday read | Data / decision | Jakub Nguyen *(proposed)* | 9/24 | Cause stated; baseline noted in the Daily Monitoring doc | sql/06, sql/11, JPROD-955 | Open |
| F15 | Web checkout views per buy-click +12% vs the previous Tuesday on 9/22 with web orders flat. Check hourly web checkout views vs purchases against the JPROD-969 window (11:42–15:10 UTC) for reloads or errors | Incident / tech | Jakub Nguyen *(proposed)* | 9/24 | Explained, or a ticket raised with an owner | junoHourly by hour; JPROD-969 | Open |
| F16 | App push impressions −9.7% Tue/Mon vs +1% normal on 9/22. Confirm push send volume on 9/21 vs 9/22 | Channel | Managed Channels *(proposed)* | 9/24 | Send volume confirmed; cause stated if lower | push_datamart (needs access, F10) | Open |
